...
from witch.fitter import make_parser, load_tods, process_tods, load_config, get_outdir
...
if args.nosub: cfg["sub"] = False
if args.nofit: cfg["fit"] = False

todvec = load_tods(cfg)

lims    = todvec.lims()
pixsize = np.deg2rad(cfg["coords"]["dr"]/3.60E+03) # np.deg2rad(1.00/3600)
