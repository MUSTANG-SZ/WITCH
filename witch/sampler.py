...
def sample(model,todvec,skymap,nwalk=100,nstep=1000,nburn=500,usepval=True,**kwargs): 
...
def update_noise(model,todvec,skymap,samples,noise_class,*noise_args,**noise_kwargs):
    ...
    for ti, tod in enumerate(tods):
        ...
        x = tod[0].copy()
        y = tod[1].copy()
        pred_tod = fm.bilinear_interp(x,y,model.xyz[0].ravel(),model.xyz[1].ravel(),pred)
