...
def faster_get_chis(m, A, rhs, v, weight, dd = None):
    """
    ...
    A : NDArray[np.floating]
        Currently the pointing matrix but hopefully this can be reduced to just some parameters
        with pointing recontstructed on GPU
    ...
    """
    Am = jnp.dot(A, m)
    Am = Am.at[:,0].set((jnp.sqrt(0.5)*Am)[:,0])
    Am = Am.at[:,-1].set((jnp.sqrt(0.5)*Am)[:,-1])
    Am_rot = jnp.dot(v,Am)
